"""Tests for GEC-11 structured reference text and retrieval functions."""
...
class TestLoadChapter:
    ...
    @pytest.mark.parametrize("chapter", list(range(1, 8)))
    def test_all_chapters_load(self, chapter):
        """All GEC-11 chapter JSON files load without error."""
        try:
            data = load_chapter("gec_11", chapter)
    ...
    def test_chapter4_reference_id(self):
        """Chapter 4 has correct reference ID."""
        try:
            data = load_chapter("gec_11", 4)
        except FileNotFoundError:
            pytest.skip("Chapter 4 not yet created")
        assert data["reference_id"] == "FHWA-NHI-10-024"

    def test_chapter4_has_mse_content(self):
        """Chapter 4 covers MSE wall design."""
        try:
            data = load_chapter("gec_11", 4)
        except FileNotFoundError:
            pytest.skip("Chapter 4 not yet created")
        title_lower = data["chapter_title"].lower()
        assert any(
            word in title_lower
            for word in ("mse", "design", "wall", "lrfd")
        )

    def test_nonexistent_chapter(self):
        """Loading a non-existent chapter raises FileNotFoundError."""
        with pytest.raises(FileNotFoundError):
            load_chapter("gec_11", 99)
...
class TestRetrieveSection:
    ...
    def test_retrieve_existing_section(self):
        ...
        chapters = list_chapters("gec_11")
        if not chapters:
            pytest.skip("No GEC-11 chapters available")
        first_ch = chapters[0]
        if not first_ch["sections"]:
            pytest.skip("First chapter has no sections")
        section_id = first_ch["sections"][0]["section_id"]
        section = retrieve_section("gec_11", section_id)
        assert "title" in section
        assert "body" in section
    ...
    def test_nonexistent_section(self):
        ...
        with pytest.raises(KeyError):
            retrieve_section("gec_11", "99.99.99")
...
class TestSearchSections:
    ...
    def test_search_returns_results(self):
        ...
        results = search_sections("gec_11", "MSE wall")
        if not results:
    ...
    def test_search_reinforcement(self):
        """Searching for 'reinforcement' returns results."""
        results = search_sections("gec_11", "reinforcement")
        if not results:
    ...
    def test_search_no_results(self):
        ...
        results = search_sections("gec_11", "xyzzy_nonexistent_term_abc")
        assert results == []
...
class TestListChapters:
    ...
    def test_chapter_entries_have_fields(self):
        ...
        chapters = list_chapters("gec_11")
        if not chapters:
            pytest.skip("No GEC-11 chapters available yet")
        for ch in chapters:
