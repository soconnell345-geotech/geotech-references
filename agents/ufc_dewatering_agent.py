"""
UFC Dewatering Agent — Foundry-style wrapper for UFC 3-220-05 lookups.

Wraps dewatering well flow equations (Thiem/Dupuit), radius of
influence, wellpoint spacing, superposition, and dewatering method
selection tables.

Three entry-point functions:
  1. ufc_dewatering_agent           - Run a UFC 3-220-05 function
  2. ufc_dewatering_list_methods    - Browse available functions by category
  3. ufc_dewatering_describe_method - Get detailed parameter docs for a specific function

## FOUNDRY SETUP
Requires: `pip install geotech-references`
Import: `from agents.ufc_dewatering_agent import ufc_dewatering_agent, ufc_dewatering_list_methods, ufc_dewatering_describe_method`
"""
...
def _load_registry():
    ...
    from geotech_references.ufc_dewatering import equations
    from geotech_references.ufc_dewatering import tables

    _METHOD_REGISTRY = {}
    _METHOD_INFO = {}

    _lookup_modules = [
        (equations, "equations", "UFC 3-220-05 dewatering flow equations"),
        (tables, "tables", "UFC 3-220-05 dewatering method and permeability tables"),
    ]
    ...
    for _mod, _category_prefix, _ref in _lookup_modules:
        for _name, _func in inspect.getmembers(_mod, inspect.isfunction):
            ...
            cat = f"UFC Dewatering {_category_prefix.title()}"
...
@function
def ufc_dewatering_agent(method: str, parameters_json: str) -> str:
    """
    UFC 3-220-05 Dewatering and Groundwater Control tool.

    Provides well flow equations (Thiem confined, Dupuit unconfined),
    radius of influence (Sichardt), wellpoint spacing, equivalent well
    radius, superposition of drawdown, and dewatering method selection.

    Parameters:
        method: The function name. Use ufc_dewatering_list_methods() to browse.
    ...
    if method not in METHOD_REGISTRY:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use ufc_dewatering_list_methods() to see available methods."
        })
...
@function
def ufc_dewatering_list_methods(category: str = "") -> str:
    """
    Lists available UFC 3-220-05 dewatering functions by category.

    Parameters:
        category: Optional filter (e.g. 'well', 'drawdown', 'permeability').
...
@function
def ufc_dewatering_describe_method(method: str) -> str:
    """
    Returns detailed documentation for a UFC 3-220-05 dewatering function.

    Parameters:
        method: The method name (e.g. 'thiem_confined_flow_m3_per_s',
    ...
    if method not in METHOD_INFO:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use ufc_dewatering_list_methods() to browse."
        })
