"""
GEC-7 Agent — Foundry-style wrapper for GEC-7 reference text and lookups.

Wraps reference text retrieval, figure lookups, and table lookups from
FHWA-NHI-14-007 (GEC-7), Soil Nail Walls.

Three entry-point functions:
  1. gec7_agent           - Run a GEC-7 function (text retrieval or lookup)
  2. gec7_list_methods    - Browse available functions by category
  3. gec7_describe_method - Get detailed parameter docs for a specific function
"""
...
@function
def gec7_agent(method: str, parameters_json: str) -> str:
    """
    GEC-7 soil nail wall reference and lookup tool.

    Provides access to FHWA-NHI-14-007 (GEC-7) reference text and
    design table/figure lookups for soil nail walls. Capabilities include:

    - Reference text retrieval (sections, search, chapter listing)
    - Nominal bond strength for soil nails (coarse/fine-grained soils, rock)
    - Pullout resistance per unit length
    - SPT correlations for cohesionless soils
    - Elastic properties from soil type or SPT
    - AASHTO seismic site coefficients (F_PGA, F_v)
    - ASD factors of safety for all limit states
    - LRFD resistance factors (Tables 5.4-5.11)
    - Wall displacement estimation parameters

    Parameters:
        method: The function name. Use gec7_list_methods() to browse.
        parameters_json: JSON string of parameters. Use gec7_describe_method()
                        for details.
    ...
    if method not in METHOD_REGISTRY:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use gec7_list_methods() to see available methods."
        })
...
@function
def gec7_list_methods(category: str = "") -> str:
    """
    Lists available GEC-7 functions organized by category.

    Parameters:
        category: Optional filter. Partial match on category name
                  (e.g. 'text', 'figures', 'tables', 'bond',
                  'pullout', 'resistance', 'seismic').
...
@function
def gec7_describe_method(method: str) -> str:
    """
    Returns detailed documentation for a GEC-7 function.

    Parameters:
        method: The method name (e.g. 'table_4_4a_bond_strength_coarse',
                'figure_4_3_friction_angle_spt', 'search_sections').
    ...
    if method not in METHOD_INFO:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use gec7_list_methods() to browse."
        })
