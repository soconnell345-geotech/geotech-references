"""
UFC Backfill Agent — Foundry-style wrapper for UFC 3-220-04N lookups.

Wraps compaction criteria, material classification, lift thickness,
equipment selection, drainage requirements, compaction-induced
pressure, filter criteria, and relative compaction checks.

Three entry-point functions:
  1. ufc_backfill_agent           - Run a UFC 3-220-04N function
  2. ufc_backfill_list_methods    - Browse available functions by category
  3. ufc_backfill_describe_method - Get detailed parameter docs for a specific function

## FOUNDRY SETUP
Requires: `pip install geotech-references`
Import: `from agents.ufc_backfill_agent import ufc_backfill_agent, ufc_backfill_list_methods, ufc_backfill_describe_method`
"""
...
# ---------------------------------------------------------------------------
# Import UFC backfill modules
# ---------------------------------------------------------------------------
from geotech_references.ufc_backfill import equations
from geotech_references.ufc_backfill import tables
...
_LOOKUP_MODULES = [
    (equations, "equations", "UFC 3-220-04N compaction and filter equations"),
    (tables, "tables", "UFC 3-220-04N compaction and backfill tables"),
]
...
for _mod, _category_prefix, _ref in _LOOKUP_MODULES:
    for _name, _func in inspect.getmembers(_mod, inspect.isfunction):
        if _name.startswith("_"):
            continue
        cat = f"UFC Backfill {_category_prefix.title()}"
...
@function
def ufc_backfill_agent(method: str, parameters_json: str) -> str:
    """
    UFC 3-220-04N Backfill for Subsurface Structures tool.

    Provides compaction criteria, material classification, equipment
    selection, filter design criteria, and compaction-induced lateral
    pressure calculations.

    Parameters:
        method: The function name. Use ufc_backfill_list_methods() to browse.
    ...
    if method not in METHOD_REGISTRY:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use ufc_backfill_list_methods() to see available methods."
        })
...
@function
def ufc_backfill_list_methods(category: str = "") -> str:
    """
    Lists available UFC 3-220-04N backfill functions by category.

    Parameters:
        category: Optional filter (e.g. 'compaction', 'filter', 'drainage').
...
@function
def ufc_backfill_describe_method(method: str) -> str:
    """
    Returns detailed documentation for a UFC 3-220-04N backfill function.

    Parameters:
        method: The method name (e.g. 'table_compaction_requirements',
    ...
    if method not in METHOD_INFO:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use ufc_backfill_list_methods() to browse."
        })
