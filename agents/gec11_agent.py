"""
GEC-11 Agent — Foundry-style wrapper for GEC-11 reference text and lookups.

Wraps reference text retrieval, figure lookups, and table lookups from
FHWA-NHI-10-024 (GEC-11), Mechanically Stabilized Earth Walls and
Reinforced Soil Slopes.

Three entry-point functions:
  1. gec11_agent           - Run a GEC-11 function (text retrieval or lookup)
  2. gec11_list_methods    - Browse available functions by category
  3. gec11_describe_method - Get detailed parameter docs for a specific function
"""
...
# ---------------------------------------------------------------------------
# Import GEC-11 modules
# ---------------------------------------------------------------------------
from geotech_references import _retrieval
from geotech_references.gec_11 import figures
from geotech_references.gec_11 import tables
...
def retrieve_section(section_id: str) -> dict:
    """Retrieve a specific section from GEC-11 by its ID (e.g., '4.3.1').
    ...
    ----------
    section_id : str
        Section identifier (e.g., '4.3.1', '2.1').
    ...
    dict
        The section data.
    """
    return _retrieval.retrieve_section("gec_11", section_id)


def search_sections(query: str) -> list:
    """Keyword search across all GEC-11 sections.
    ...
    """
    return _retrieval.search_sections("gec_11", query)


def list_chapters() -> list:
    """List all GEC-11 chapters and their section IDs.
    ...
    """
    return _retrieval.list_chapters("gec_11")


def load_chapter(chapter: int) -> dict:
    """Load a full GEC-11 chapter JSON file.

    Parameters
    ----------
    chapter : int
        Chapter number (1-7 for Volume I).
    ...
    """
    return _retrieval.load_chapter("gec_11", chapter)
...
_LOOKUP_MODULES = [
    (figures, "figures", "FHWA-NHI-10-024, Chapter 4 Figures"),
    (tables, "tables", "FHWA-NHI-10-024, Chapters 2-4 Tables"),
]
...
for _name, _func in _TEXT_METHODS.items():
    METHOD_INFO[_name] = _extract_info(
        _func, "Text Retrieval", "FHWA-NHI-10-024"
    )
...
for _mod, _category_prefix, _ref in _LOOKUP_MODULES:
    for _name, _func in inspect.getmembers(_mod, inspect.isfunction):
        if _name.startswith("_"):
            continue
        cat = f"GEC-11 {_category_prefix.title()}"
...
@function
def gec11_agent(method: str, parameters_json: str) -> str:
    """
    GEC-11 MSE wall and reinforced soil slope reference and lookup tool.

    Provides access to FHWA-NHI-10-024 (GEC-11) reference text and
    design table/figure lookups for MSE walls. Capabilities:

    - Reference text retrieval (sections, search, chapter listing)
    - Minimum reinforcement length (Table 2-1)
    - Minimum embedment depth (Table 2-2)
    - Select fill gradation requirements (Table 3-1)
    - Electrochemical limits for steel/geosynthetics (Tables 3-3, 3-4)
    - Pullout capacity parameters F* and alpha (Table 3-6)
    - Galvanization thickness (Table 3-7)
    - Steel corrosion rates (Table 3-8)
    - Installation damage reduction factors RF_ID (Table 3-9)
    - PET durability reduction factors RF_D (Table 3-11)
    - LRFD load combinations (Table 4-1)
    - Permanent load factors gamma_p (Table 4-2)
    - Traffic surcharge equivalent height (Table 4-4)
    - External stability resistance factors (Table 4-5)
    - Bearing capacity factors Nc, Nq, Ngamma (Table 4-6)
    - Internal stability resistance factors (Table 4-7)
    - Kr/Ka ratio vs depth (Figure 4-10)

    Parameters:
        method: The function name. Use gec11_list_methods() to browse.
        parameters_json: JSON string of parameters. Use gec11_describe_method()
                        for details.
    ...
    if method not in METHOD_REGISTRY:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use gec11_list_methods() to see available methods."
        })
...
@function
def gec11_list_methods(category: str = "") -> str:
    """
    Lists available GEC-11 functions organized by category.

    Parameters:
        category: Optional filter. Partial match on category name
                  (e.g. 'text', 'figures', 'tables', 'lrfd',
                  'corrosion', 'pullout', 'bearing').
...
@function
def gec11_describe_method(method: str) -> str:
    """
    Returns detailed documentation for a GEC-11 function.

    Parameters:
        method: The method name (e.g. 'table_4_6_bearing_capacity_factors',
                'figure_4_10_kr_ka_ratio', 'search_sections').
    ...
    if method not in METHOD_INFO:
        ...
        return json.dumps({
            "error": f"Unknown method '{method}'. "
                     "Use gec11_list_methods() to browse."
        })
